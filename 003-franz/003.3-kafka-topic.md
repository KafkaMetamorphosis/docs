# Kafka Topic and Topic Revision

Status: **draft**

> **Alignment (ADR 001-ux, 002).** **Topic Claim → Kafka Topic**; its parent **Topic Definition → Async Channel**. Topic Revision keeps its name. The authoritative wire contract is `../../api/proto/franz/v1/kafka.proto` (`KafkaTopic`, `TopicRevision`, `KafkaTopicState`, `RevisionState`) and `fleet.proto` (the agent pull/report API); the `/api/v0` tables below are superseded. Expansion is no longer a separate engine — see `003.0-franz.md`.
...
| `cluster-id` | `uuid` | Yes | The id of the cluster it will be in. |
...
| Verb | Path | Description |
|---|---|---|
| `GET` | `/api/v0/clusters/:cluster-name/poll-pending-reconciliations` | Return all active claims (with desired config and optional pending revision) for the cluster. |
| `PUT` | `/api/v0/clusters/:cluster-name/inform-reconciliation-status` | Report a batch of revision outcomes. |
