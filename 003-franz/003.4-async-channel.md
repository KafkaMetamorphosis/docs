...
A channel is **sharded** into `channel_partitions` Kafka Topics (`003.6`), each
placed on a Kafka Cluster whose labels satisfy the channel's `franz.affinity/*`
labels (`003.7`). The channel is 1:N to Kafka Topic; each shard maps to one real
topic in one cluster. Shards of the same channel may land on different clusters.
Producers/consumers route to a shard by key hash through the Franz SDK.

Franz materialises the shard `kafka_topic` rows **at placement, not at
`CreateAsyncChannel`** (ADR-API-009): `CreateAsyncChannel` records only the
channel and its `channel_partitions` count; placement (`003.7`) creates each
shard the moment it can assign it a cluster, seeding `partitions` /
`replication_factor` / materialised config from that cluster's
`cluster_configuration`. A channel with no eligible cluster has **no shards yet**;
`GetAsyncChannel` reports "M of N shards placed".
...
- **Placement** — driven entirely by the channel's `franz.*` labels against
  cluster labels (`003.7`). Placement **creates** each shard `kafka_topic` row
  when it assigns it a cluster (ADR-API-009); a channel with no matching cluster
  has no shards yet. Editing the labels so a placed shard's cluster no longer
  matches marks it *misplaced* and queues a migration (`003.7`, `003.13`).
