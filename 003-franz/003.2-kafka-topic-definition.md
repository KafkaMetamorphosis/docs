...
| `status` | `status` | Yes | Status of the claim |
| `labels` | `map<string, string>` | No | Arbitrary key-value metadata for categorization and filtering. Defaults to an empty map. |

### Topic Definition state machine transitions:
...
|---|---|---|---|
| `Active` | `Pending` | `PendingCreation` | Claim exists, topic being created on Kafka for the first time |
| `Active` | `Pending` | `PendingUpdate` | Topic exists on Kafka, config change in flight |
...
| `Paused` | `Paused` | `PendingCreation` / `PendingUpdate` | Was in flight when paused — Gregor Samsa will not pick up work |
