...
> **Alignment (ADR 001-ux, 002).** "Topic Definition" → **Async Channel**; "Topic Claim" → **Kafka Topic**. Selection is now a step of Async Channel create/update, not a standalone expansion engine (`003.6` is superseded). The label-selector grammar is unified with ADR 001-ux: `key=value, key IN (a, b)`; the reserved `franz.*` label mechanics (`franz.affinity/selector`, `franz.affinity/shard-size`, `franz.taint`, `franz.taint/toleration`) are unchanged. No wire types live here — placement inputs are fields on `async_channel.proto`.
