# Async Channel and Kafka Topic

Status: **draft**

> **Alignment (ADR 001-ux, 002).** This document is renamed in intent: **Topic Definition → Async Channel** (`async_channel.proto`), **Topic Claim → Kafka Topic** (`kafka.proto`). Topic Configuration and Topic Revision keep their names. The authoritative wire contract is `../../api/proto/franz/v1/`; the `/api/v0` endpoint tables and state-machine syntax below are superseded by the proto services and enums. A deeper rewrite of this file is pending.
...
| `status` | `string` | Yes | Lifecycle status of the definition. |
| `expansion-status` | `string` | Yes | The status of expansion: Expanded or PendingExpansion |
| `labels` | `map<string, string>` | No | Arbitrary key-value metadata for categorization and filtering. Defaults to an empty map. |
