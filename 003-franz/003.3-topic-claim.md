...
| `operation` | `string` | Yes | The intended operation: `create`, `update`, or `delete`. Determines the terminal state after a successful outcome. |
...
stateDiagram-v2
    ...
    Error     --> Pending : Manual retry after max attempts
    ...
    "failed": [
        {
            ...
            "message": "Latest revision is in 'Pending' state, cannot retry"
        }
    ]
}
```
...
|---|---|---|---|
| `Active` | `Pending` | `Pending` (operation: `create`) | Claim exists, topic being created on Kafka for the first time |
| `Active` | `Pending` | `Pending` (operation: `update`) | Topic exists on Kafka, config change in flight |
...
| `Paused` | `Paused` | `Pending` | Was in flight when paused — Gregor Samsa will not pick up work |
| `Paused` | `Paused` | `Error` | Failed before pausing, frozen in error state |
| `Deleted` | `Pending` | `Pending` (operation: `delete`) | Deletion requested, topic removal in flight on Kafka |
...
When Franz receives an inform call, it looks up the revision's `operation` field and combines it with the reported outcome to derive both the revision terminal state and the new claim state:

| Revision operation | Outcome | Revision terminal state | Claim state |
|---|---|---|---|
| `create` | `success` | `Created` | `Ready` |
| `update` | `success` | `Updated` | `Ready` |
| `delete` | `success` | `Deleted` | `Deleted` |
| `create` | `failure` | `Error` | `Error` |
| `update` | `failure` | `Error` | `Error` |
| `delete` | `failure` | `Error` | `Error` |
