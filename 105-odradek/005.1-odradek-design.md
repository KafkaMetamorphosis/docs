...
It is a metric that expose all topic configuration like partitions, retentions and all other configs. As well as topic location (the broker id), and so one.

It is exposed by a dedicated job, that runs in the configured time at `topic_config.scrape_interval` in the config.
