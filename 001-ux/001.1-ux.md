...
| **Actions** | What to run against every matched resource when the limit is crossed: `add_label(key, value)`, `set_status(status)`, `update_field(field, value)`, `increase_field_by(field, amount)`, `decrease_field_by(field, amount)`. |
...
Every channel carries **one access policy document**, in the style of an S3 bucket policy. It is a list of statements; each statement names a **principal** — a client ORN, a label selector (`=` and `IN (…)`), or both — and grants `Read`, `Write`, or both. Franz is **zero trust**: a client that matches no statement has no access.

A client's effective permission on a channel is what the matching statements grant. The client itself carries no role, so there is nothing to intersect — the policy is the sole source of permission.
...
- [Kafka Cluster Management](../003-franz/003.1-kafka-cluster.md)
- [Async Channel and Kafka Topic](../003-franz/003.2-async-channel.md)
- [Kafka Topic and Topic Revision](../003-franz/003.3-kafka-topic.md)
- [Topic Cluster Selection](../003-franz/003.4-topic-cluster-selection.md)
- [Governance](../003-franz/003.5-governance.md)
